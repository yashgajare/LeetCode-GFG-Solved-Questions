...
class Solution {
  public:
    ...
    bool isCBT(struct Node* root, int i, int nc){
        if(root==NULL) return true;
        
        if(i>=nc) return false;
        
        else{
            bool left = isCBT(root->left, 2*i+1, nc);
            bool right = isCBT(root->right, 2*i+2, nc);
            return left&&right;
        }
    }
    ...
    bool isMaxOrder(struct Node* root){
        ...
        if(root->left!=NULL && root->right==NULL) return root->data > root->left->data;
